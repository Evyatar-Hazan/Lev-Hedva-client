...
// 🧪 בדיקות E2E לאפליקציית Lev-Hedva

test.describe('Lev-Hedva Application E2E Tests', () => {
  
  test.beforeEach(async ({ page }) => {
    // הגדרות בסיסיות לכל בדיקה
    await page.goto('http://localhost:3000');
    
    // בדיקה שהאפליקציה נטענה
    await expect(page).toHaveTitle(/Lev Hedva/i);
  });

  test.describe('🔐 בדיקות התחברות', () => {
    
    test('should display login page correctly', async ({ page }) => {
      // ודא שעמוד ההתחברות נטען
      ...
      await expect(page.getByRole('button', { name: 'היכנס' })).toBeVisible();
    });
  ...
  test.describe('🔗 בדיקות תקשורת עם השרת', () => {
    ...
    test('should show connection status', async ({ page }) => {
      ...
      if (await connectionCheck.isVisible()) {
        await expect(page.getByText('השרת לא פועל').or(page.getByText('קליינט פעיל'))).toBeVisible();
      }
    });
